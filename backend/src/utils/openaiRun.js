 ...
 */
export const resolveOpenAIConfig = async (userId, developerMode = false) => {
  let apiKey = null;
  ...
  if (userId) {
    const withKey = await getSettingsWithKey(userId);
 ...
 */
export const runAgent = async (orchestratorAgent, agentInput, options = {}) => {
  const { stream = false, userId = null, developerMode = false } = options;
  const config = await resolveOpenAIConfig(userId, developerMode);

  if (!config.apiKey) {
    throw new Error('OpenAI API key not configured. Set it in Settings or in server .env.');
  }
